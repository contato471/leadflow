...
const NAV = [
  { sec:'CRM',       href:'/crm/dashboard',       label:'Dashboard',     icon:'ti-layout-dashboard', mobileLabel:'Dashboard' },
  { sec:'CRM',       href:'/crm/funil',            label:'Funil',         icon:'ti-layout-columns',   mobileLabel:'Funil' },
  { sec:'CRM',       href:'/crm/clientes',         label:'Clientes',      icon:'ti-users',            mobileLabel:'Clientes' },
  { sec:'Comercial', href:'/crm/leads',            label:'Leads',         icon:'ti-bolt',             mobileLabel:'Leads' },
  { sec:'Comercial', href:'/crm/corretores',       label:'Corretores',    icon:'ti-id-badge',         mobileLabel:'Corretores' },
...
// Bottom nav mobile: só os mais importantes
const MOBILE_NAV = [
  { href:'/crm/dashboard',  label:'Dashboard', icon:'ti-layout-dashboard' },
  { href:'/crm/funil',      label:'Funil',     icon:'ti-layout-columns' },
  { href:'/crm/leads',      label:'Leads',     icon:'ti-bolt' },
  { href:'/crm/estoque',    label:'Estoque',   icon:'ti-map-2' },
  { href:'/crm/propostas',  label:'Propostas', icon:'ti-file-text' },
]
...
export default function CrmLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display:'flex', height:'100dvh', overflow:'hidden', background:'var(--bg-3)' }}>
      ...
      {/* Mobile Sidebar Overlay (menu completo) */}
      {mobileOpen && (
              ...
              {sections.map(sec => (
                <div key={sec}>
                  ...
                  {NAV.filter(n => n.sec === sec).map(item => {
                    const active = pathname === item.href || (item.href.length > 5 && pathname.startsWith(item.href))
                    return (
                      <a key={item.href} href={item.href} onClick={() => setMobileOpen(false)} style={{ display:'flex', alignItems:'center', gap:'9px', padding:'9px', fontSize:'14px', color:active?'var(--text)':'var(--text-2)', borderRadius:'var(--radius)', margin:'1px 0', background:active?'var(--bg-2)':'transparent', fontWeight:active?500:400, textDecoration:'none', borderLeft:active?'2px solid var(--teal)':'2px solid transparent' }}>
                        <i className={`ti ${item.icon}`} style={{ fontSize:'18px', flexShrink:0 }} aria-hidden="true"></i>
                        <span>{item.label}</span>
                      </a>
                    )
                  })}
                  {sec !== 'Sistema' && <div style={{ height:'0.5px', background:'var(--border)', margin:'6px 2px' }}/>}
                </div>
              ))}
            </div>
            {sidebarFooter}
